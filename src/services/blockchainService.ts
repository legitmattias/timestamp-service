...
const provider = new JsonRpcProvider('http://127.0.0.1:8545') // Local blockchain
 ...
 */
export const saveNoteOnBlockchain = async (hash: string): Promise<string> => {
  try {
 ...
 */
export const getNoteFromBlockchain = async (hash: string): Promise<number> => {
  try {
